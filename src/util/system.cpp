...
const char * const BITCOIN_CONF_FILENAME = "tommycoin.conf";
...
#else
    const char* pszModule = "tommycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tommycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tommycoin";
#else
    // Unix
    return pathRet / ".tommycoin";
#endif
#endif
}
